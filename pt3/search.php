...
$search_value=$_POST["search"];
          $servername="lrgs.ftsm.ukm.my"; // Host name 
...
if($con->connect_error){
    echo 'Connection Faild: '.$con->connect_error;
        ...
        $sql="select * from tbl_products_a161180_pt2 where PRODUCT_NAME like '%$search_value%' or BRAND like '%$search_value%' or COLOR like '%$search_value%' ";

        $res=$con->query($sql);

        while($row=$res->fetch_assoc()){
            // echo 'Product Name :  '.$row["PRODUCT_NAME"];
            // echo 'Product Brand :  '.$row["BRAND"];
            // echo 'Product Color :  '.$row["COLOR"];
...
try {
  ...
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->prepare("select * from tbl_products_a161180_pt2 where PRODUCT_NAME like '%$search_value%' or BRAND like '%$search_value%' or COLOR like '%$search_value%' LIMIT $start_from, $per_page");
  $stmt->execute();
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("select * from tbl_products_a161180_pt2 where PRODUCT_NAME like '%$search_value%' or BRAND like '%$search_value%' or COLOR like '%$search_value%'");
            $stmt->execute();
            $result = $stmt->fetchAll();
